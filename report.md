...
- **输入边界**
    ...
    - 测试步骤：
        1. 在eggSpinner中手动输入 0 并触发购买（或失焦提交编辑）；观察 spinner 的最终值与购买行为。
        2. 在eggSpinner中手动输入 -1 并触发购买；观察行为。
        3. 在eggSpinner中输入 999 （超出上限）并触发购买；观察最终计价与控件值。
    - 预期结果：
...
- 测试步骤：
    1. 在运行时通过 `GameDataManager` 将所有宠物的 `isAlive` 设置为 `false`，或通过数据库将 `pet` 表中的 `IsAlive` 字段全部设为 0 并重新加载存档。
    2. 在主界面或卧室界面点击“进入迷宫/开始探索”按钮，触发探索入口逻辑。
    3. 观察界面行为：是否阻止跳转到迷宫界面（`maze.fxml`），并弹出友好提示框；记录提示文本。
    4. 验证后端校验：确认入口逻辑（如 `MazeController` 或调用入口处）在进入前调用 `GameDataManager` 的校验方法（例如 `hasAlivePokemon()` 或等效方法），并在校验失败时不执行探索初始化（例如场景切换、资源扣减、计时器启动）。
    5. 恢复场景：将至少一只宠物 `isAlive` 设为 `true`，再次点击进入探索以确认正常跳转。
- 预期结果：
    - 界面阻止进入迷宫并弹出友好提示，建议提示文本为“提示：没有可出战的宝可梦，无法进入探索”。
    - 控制器/入口逻辑应调用明确的前置校验方法并在校验失败时立即返回，不执行探索相关的初始化动作（如扣除体力、启动计时器或切换场景）。
    - 应在日志或控制台记录一条可读的调试信息（例如："Enter exploration blocked: no alive pokemon"）。
    - 数据库与 `GameDataManager` 的运行时状态保持不变（无副作用）。
- 实际结果：待执行/填写

### 4.1.5 战斗时校验
- 用例说明：验证战斗中技能使用的 PP 校验，确保玩家和敌方均不能使用消耗超过当前 PP 的技能，并验证系统在 PP 不足时的自动处理策略。
- 测试步骤：
    1. 在战斗中将当前出战宝可梦的某个技能的 `currentPp` 设置为小于该技能消耗 (`ppCost`) 的值（例如技能消耗为 3，设置为 2），尝试使用该技能。
    2. 检查 `BattleStepResult` 或战斗日志，确认是否返回“PP 不足”或等价提示；确认该回合是否按预期被跳过并允许玩家选择其他动作（技能/道具/替换）。
    3. 将当前宝可梦所有技能的 `currentPp` 都设置为 0，观察系统行为：是否执行自动跳过、尝试 PP 恢复、提示替换或判定该方无法继续战斗并结束战斗。
    4. 重复上述用例以覆盖敌方在 PP 不足时的行动逻辑，验证敌方行为的鲁棒性和日志记录。
- 预期结果：
    - 当玩家尝试使用 PP 不足的技能时，界面应禁用该技能或在点击后显示“PP 不足”的提示，`BattleManager` 应返回失败的 `BattleStepResult` 并允许玩家选择其他操作，且不会产生未捕获异常。
    - 当所有技能均无 PP 时，系统应按设计优先级执行自动处理（例如自动跳过、触发 PP 恢复、提示替换或判定失败并结束战斗），并在战斗日志中记录清晰原因；若无法继续战斗，应触发战斗结束与结算流程。
- 实际结果：待执行/填写
## 4.2 完整探索游玩流程（战斗以及探索系统）
- 
